  ...
  useEffect(() => {
    if (mapData && spiData && !loading) {
      ...
      function ready(data) {
        ...
        const zoom = d3.zoom()
          .translateExtent([[-.15 * width, -.15 * height], [width * 1.25, height * 1.25]])
          .scaleExtent([1, 10])
          .on('zoom', zoomed)
        ...
        function zoomToTarget(value) {
          ...
          if (!target || svg.select(targetId).empty()) return zoomToCenter(); //catches clicks on borders

          let bounds = svg.select(targetId).node().getBoundingClientRect();
        ...
        //ZoomToCenter and Tooltip Center targets
        g.selectAll(`.toolTipTarget`)
          //** Filter so we only zoom to valid countries  */
          .data(mapFeatures.filter(d => d.properties.mapId !== "ATA"))
          .join('circle')
          ...
          .attr("r", 1) //Safari requires an radius to have a value or BBox fails
          .attr("fill", "none") //Hide the Tooltip/Zoom target
